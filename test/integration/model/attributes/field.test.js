    ...
    describe('field', () => {
      ...
      it('should support renaming of sequelize method fields', async function () {
        ...
        if (dialect === 'mssql') {
          ...
          findAttributes = [
            Sequelize.literal(
              'CAST(CASE WHEN EXISTS(SELECT 1 FROM DUAL) THEN 1 ELSE 0 END AS NUMBER) AS "someProperty"'
            ),
            [
              Sequelize.literal('CAST(CASE WHEN EXISTS(SELECT 1 FROM DUAL) THEN 1 ELSE 0 END AS NUMBER)'),
              'someProperty2'
            ]
          ];
        } else {
