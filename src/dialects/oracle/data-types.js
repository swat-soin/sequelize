    ...
    if (this._length) {
      if (typeof this._length === 'string') {
        switch (this._length.toLowerCase()) {
          case 'tiny':
            warn(
              'ORACLE does not support TEXT with the `length` = `tiny` option. `NVARCHAR(256)` will be used instead.'
            );
            return 'NVARCHAR2(256)';
          case 'medium':
            warn(
              'ORACLE does not support TEXT with the `length` = `medium` option. `NVARCHAR(2000)` will be used instead.'
            );
            return 'NVARCHAR2(2000)';
          case 'long':
            warn(
              'ORACLE does not support TEXT with the `length` = `long` option. `NVARCHAR(4000)` will be used instead.'
            );
